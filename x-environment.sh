...
if [ -z "$GXG" ]; then
  echo "This script wants to \`git stash\` any files in the home directory that conflict with github.com/hgto/dots"
...
if [ "$GXG" = 1 ]; then 
  ...
  vcsh Xdots remote add origin gh:hgto/Xdots
fi
